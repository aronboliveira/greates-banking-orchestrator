...
class GlobalExceptionHandlerTest {
    ...
    void handleResponseStatus_shouldPreserveStatusAndReason() {
        ResponseEntity<ErrorResponse> response = handler.handleResponseStatus(
                new ResponseStatusException(FORBIDDEN, "role does not allow this action"));

        assertThat(response.getStatusCode().value()).isEqualTo(403);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().status()).isEqualTo(403);
        assertThat(response.getBody().error()).isEqualTo("403 FORBIDDEN");
        assertThat(response.getBody().message()).isEqualTo("role does not allow this action");
    }
