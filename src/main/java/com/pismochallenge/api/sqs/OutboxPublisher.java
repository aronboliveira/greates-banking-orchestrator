package com.pismochallenge.api.sqs;
 ...
 *
 * <p>Polls every {@code pismo.outbox.fixed-delay-ms} (default 1000 ms) and
 * publishes up to {@code pismo.outbox.batch-size} (default 50) rows per tick.
 */
@Component
@Profile("eks-outbox")
public class OutboxPublisher {
    ...
    public OutboxPublisher(OutboxRepository repository,
                           ...
                           @Value("${pismo.outbox.batch-size:50}") int batchSize) {
    ...
    @Scheduled(fixedDelayString = "${pismo.outbox.fixed-delay-ms:1000}")
    @Transactional
    public void publish() {
